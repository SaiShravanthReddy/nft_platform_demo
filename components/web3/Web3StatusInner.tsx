...
function Web3StatusInner() {
  ...
  const formattedBalance = nativeBalance ? `${parseFloat(nativeBalance.formatted).toFixed(4)} ${nativeBalance.symbol}` : null;
      ...
      >
        ...
        >
          <p className={classNames(isBABTHolder && 'font-medium text-black')}>
            Address: {shortenAddress(address)} Balance: {formattedBalance ?? '...'}
          </p>
